...
def get_palette_color(color_idx, max_colors):
    """Generates distinct, vibrant HSL-derived colors for noble face configurations."""
    if max_colors <= 0:
        max_colors = 1
    hue = color_idx / max_colors
    h = hue * 6.0
    c = 0.85 * (1.0 - abs(2.0 * 0.6 - 1.0))  # Chroma (Saturation: 0.85, Lightness: 0.6)
...
def compute_noble_face_colors(faces, num_vertices):
    """Greedy face-coloring algorithm ensuring no two faces sharing a vertex share a color."""
    num_faces = len(faces)
    vert_to_faces = {}
    for f_idx, face in enumerate(faces):
        for v in face:
            vert_to_faces.setdefault(v, []).append(f_idx)
    ...
    for v_faces in vert_to_faces.values():
        for i in range(len(v_faces)):
            for j in range(i + 1, len(v_faces)):
                f1, f2 = v_faces[i], v_faces[j]
                adj[f1].add(f2)
                adj[f2].add(f1)

    face_colors = [-1] * num_faces
    for i in range(num_faces):
        used = {face_colors[nb] for nb in adj[i] if face_colors[nb] != -1}
        c = 0
        while c in used:
            c += 1
        face_colors[i] = c
...
class HeadlessRenderer:
    ...
    def render(self, vertices, faces, angle_x, angle_y, is_noble=False):
        if not self.ctx:
            return None
        ...
        if is_noble:
            face_color_indices = compute_noble_face_colors(faces, len(vertices))
        ...
        vbo_solid = self.ctx.buffer(vbo_array.tobytes())
        vao_solid = self.ctx.vertex_array(
            self.prog_solid,
            [(vbo_solid, "3f 3f 3f", "in_position", "in_normal", "in_color")],
        )

        vbo_lines = self.ctx.buffer(l_data.tobytes())
        vao_lines = self.ctx.simple_vertex_array(
            self.prog_lines, vbo_lines, "in_position"
        )
...
def render_grid_view(
    vertices, solutions, tk_update_callback=None, control_flags=None, is_noble=False
):
